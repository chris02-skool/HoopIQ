...
from data import load_real_shot_data, get_shot_results
from pathlib import Path

# Check if HoopIQ system is running
def check_hoopiq_status():
    """Check if HoopIQ detection system is active"""
    stats_file = Path('/tmp/hoopiq_stats.json')
    if stats_file.exists():
        ...
        last_update = stats.get('timestamp', 0)
        is_active = (current_time - last_update) < 5000
...
# Show system status
is_active, stats = check_hoopiq_status()
if is_active:
    st.success(f"✅ Detection System Active - {stats['camera']['fps']} FPS")
    st.metric("Total Shots Detected", stats['total_makes'])
else:
    st.warning("⚠️ Detection system not running")
    st.info("Start the Rust detection system to see live data")

# Load and display shot data
shot_data = get_shot_results()
...
# - In the individual json file, add password checking for security (if possible)